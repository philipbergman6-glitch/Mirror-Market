...
**"Skipped" means unconfigured, never "the upstream died."** The two API-key-gated dict layers (`export_sales`, `eia`) write no freshness row when their key is unset — the layer genuinely never ran, and either a `success` or a `failed` row there would be a lie. That skip is decided by `DictLayer.skip_gate`, a predicate consulted **before** `fetch()` — `fetchers.export_sales.is_configured` / `fetchers.eia.is_configured`, each reading its own module's key constant so the pipeline's answer and the fetcher's own gate cannot disagree. Inferring the skip from the *result* instead (the old `if not data and layer.skip_msg`) conflated the two meanings of a bare `{}`: a FAS or EIA outage returned before `_finalize_layer` and recorded nothing at all, so it read as "you never configured this layer" (#180). With the gate ahead of the fetch, a configured layer always reaches `_finalize_layer`; both carry a `LAYER_MIN_KEYS` floor of 2+, so an all-empty run grades as a failure by the rules above.
