...
1. **A data collector** — a Python script that runs 32 operational data layers (28 numbered source groups plus four independently graded sub-layers) every weekday and saves everything into one database.
...
\* needs a free API key (`USDA_API_KEY`, `FRED_API_KEY`, `FAS_API_KEY`, `EIA_API_KEY`, and optionally `DATA_GOV_IN_API_KEY`). **26 of 32 operational layers need no private key**, and if any one contextual source fails, the rest still run and the degradation remains visible.
...
python main.py                      # run all 32 operational layers
