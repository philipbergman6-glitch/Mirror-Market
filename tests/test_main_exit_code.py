...
def test_exit_zero_when_critical_layers_succeed(stub_fetchers, monkeypatch):
    ...
    fred_series = pd.Series([1.0, 2.0], index=pd.to_datetime(["2026-01-01", "2026-01-02"]))
    monkeypatch.setattr(main, "fetch_all_series", mock.Mock(return_value={"CPI": fred_series}))
...
def test_failed_layer_writes_freshness_row(stub_fetchers, monkeypatch):
    ...
    fred_series = pd.Series([1.0], index=pd.to_datetime(["2026-01-01"]))
    monkeypatch.setattr(main, "fetch_all_series", mock.Mock(return_value={"CPI": fred_series}))
