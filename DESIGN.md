...
An inline expansion **under** the clicked contract row — same shape as the ledger drill-down, one open at a time **across the whole page** (the section renders one table per commodity, so the close-others sweep is document-wide, not per table), never a modal. The panel row sits on a cool slate inset `#EEF1F4` with a 3px `--info` (`#2F5D8F`) left border: not paper `#FAFAF7`, not the soy green, not a white card. Above the chart, two lines, both rendered whole from `app/workstation_page.py` — the template composes no part of either: a mono 10px uppercase `--info` stamp — `Third party · TradingView · delayed exchange data (~10 min, their figure)`, because the delay is TradingView's claim about their own feed, not an age this project measured — and a 11.5px muted sentence saying the table's numbers are ours on our timestamps, the chart's are theirs on theirs, and that expanding a row requests it from TradingView's servers. Chart frame 452px (352px under 768px), white, 1px `#C7CFD6`, the last 32px being the copyright line.

**The attribution below the chart is a licence condition, not decoration** (invariant 9): TradingView's Terms of Use (tradingview.com/policies/, read 2026-08-23) require the attribution kept *as originally designed and intended* — so what ships is the embed's **own** `tradingview-widget-container` / `tradingview-widget-copyright` markup, whose classes their script looks up and whose link it rewrites, never a credit line of ours. Our CSS may recolour it into the frame's palette but no rule may hide it, shrink it to nothing, or position it off-screen, and a test asserts the exact markup renders.

The chevron and row affordances are gated on a `.tv-ready` class the script adds after wiring — with script off the page loses the expander cleanly rather than offering a control that does nothing, the same gate `.drill-ready` applies on market pages. The embed is **lazy**: the panel ships empty and hidden, and the TradingView script is injected on first expand, so a page listing a dozen contract months ships zero iframes. A row whose venue has no checked symbol gets no expander at all (`app/tradingview.py`) — a chart of the wrong contract beside a right price is the wrong-number-worse-than-a-gap trade invariant 11 refuses. A failed embed says so in `--bearish` text and clears its loaded flag so reopening the row retries; an empty grey box beside a price would read as "no trading".
