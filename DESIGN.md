...
### Contract-row chart panel (workstation contract rows only)
Reshaped 2026-08-23 by [#328](https://github.com/philipbergman6-glitch/Mirror-Market/issues/328): the embedded TradingView widget this section previously specified is **dead** — their free embed widget refuses CME symbols on any plan and fell back to rendering AAPL, the exact wrong-chart-beside-a-right-price failure invariant 11 names. Nothing third-party renders on this site any more. The expander survives; what it opens is now **split territories**, chosen by the user from three options (the rejected two: the whole panel on our paper with the links as a caption line, which under-marked the boundary; and the whole panel kept slate with our chart as a white card inside, which put our data inside third-party framing).

An inline expansion **under** the clicked contract row — same shape as the ledger drill-down, one open at a time **across the whole page** (the section renders one table per commodity, so the close-others sweep is document-wide, not per table), never a modal. The panel is two stacked zones whose left-border colour is the territory line:

**Ours, on top** — paper `#FAFAF7` (`--paper`) with a 3px `--green` left border. A mono 10px uppercase `--green` stamp rendered whole from `app/workstation_page.py` — `Our data · delayed session bars · USD/MT · history since 3 Aug 2026 (498 sessions)` — over an **interactive candlestick chart** (240px, crosshair, scroll-zoom, volume histogram tucked under the bottom 18%) drawn by the **vendored lightweight-charts bundle**: TradingView's Apache-2.0 charting *library*, served from this site's own `assets/` — their code, never their data or their servers; nothing is requested at view time and the library's small attribution logo stays visible because that is what their licence terms ask of a product using the renderer. Candles are `--green` up / `--bearish` down on the transparent paper ground. The series is the contract's own Layer 11b daily bars (months deep, [#332](https://github.com/philipbergman6-glitch/Mirror-Market/issues/332)), with the Layer 11 `forward_curve` snapshots filling any close-only session the dedicated series lacks — same venue, same delayed-close vocabulary, so the merge is one kind of number. **The drawing regime is decided server-side, once**, in `app/workstation_page.py`, and shipped as `data-regime`: full bars ≥8 sessions are candles; closes alone are a `--green` line; **under 8 sessions dots only, never a joined line; under 2 no chart at all** — a stated reason in muted text instead (invariant 2). A candle renders only where open/high/low were all served (the cleaner withholds a partial trio — three-quarters of a candle would be an invented one). The old inline-SVG line survives verbatim as the fallback when the bundle is absent or throws, so the panel never goes chartless with data in hand. The stamp says when the record starts rather than letting a short axis imply a long record; points are keyed by session, so a weekend re-fetch of Friday's close is one point.

**Theirs, below** — the slate `#EEF1F4` inset with the 3px `--info` left border, now holding only **links**: a mono 10px uppercase `--info` stamp — `Third party · TradingView / Barchart · opens on their site` — then two white-card mono links to the exact contract month on TradingView's symbol page and Barchart's interactive chart (both venue-registry-gated in `app/tradingview.py`), then the 11.5px muted sentence saying everything above the rule is ours on our timestamps and the links open their pages on their data and their timing. **No delay figure anywhere**: nothing renders here, so there is no feed to characterise. Both lines render whole from Python; the template composes neither. A plain link is not an embed, so the widget-attribution markup died with the widget — TradingView's attribution condition attaches to use of their widgets and content, and a link uses neither (re-read 2026-08-23).

The chevron and row affordances are gated on a `.tv-ready` class the script adds after wiring — with script off the page loses the expander cleanly rather than offering a control that does nothing, the same gate `.drill-ready` applies on market pages. The chart is drawn on first expand from JSON already in the page — nothing is requested from anywhere, so there is no failure state to render (a missing bundle degrades to the SVG, and a missing *asset* fails the promotion contract instead of shipping quietly: `trust.site_promotion.PUBLISHED_ASSETS`). A row whose venue has no checked symbol gets no expander at all (`app/tradingview.py`): a link to the wrong contract beside a right price is the same invariant-11 trade the wrong chart was.
...
| 2026-08-23 | **The contract-row panel becomes split territories: our close-history chart over third-party links; the embedded TradingView widget is removed** | [#328](https://github.com/philipbergman6-glitch/Mirror-Market/issues/328), superseding the embed half of the #320 row above. The widget was confirmed dead on the deployed page — TradingView's free embed refuses CME symbols on any plan and fell back to AAPL — so nothing third-party renders on the site any more. Owner chose **split territories** from three options: the panel's top zone is ours (paper `--paper`, 3px `--green` border, an inline-SVG close history from our own `forward_curve` snapshots with an honest "history since" stamp, the ledger's dots-under-8 / withheld-under-2 sparseness rules), and the slate `#EEF1F4` / `--info` strip survives only as the third-party surface it actually is, holding plain links to the exact contract on TradingView and Barchart (both registry-gated, invariant 5). The rejected options: all-paper with caption links (under-marked the boundary) and the whole panel kept slate (our data inside third-party framing). The ~10-minute delay claim is gone — nothing renders here to characterise — and the widget-attribution markup died with the widget, since their attribution condition attaches to widgets and content, not links |
| 2026-08-23 | **The contract-row chart draws the contract's own daily history (Layer 11b), not just curve snapshots** | [#332](https://github.com/philipbergman6-glitch/Mirror-Market/issues/332), extending the #328 row above after the owner asked for the chart itself to open on click rather than a link-out. No visual change: same panel, same idiom, same sparseness rules — what changes is the series behind it, now months of per-contract session closes fetched through the same settlement-guarded yfinance path and merged with the snapshots (the dedicated series wins a shared session). The withheld state survives for genuinely thin histories; its text no longer cites the snapshot start date, because the record no longer begins there |
| 2026-08-23 | **The contract-row chart becomes interactive candlesticks drawn by TradingView's open-source library, vendored** | Extends the #332 row above. The owner rejected the plain SVG line and asked for TradingView charts; publishing their chart *screenshots* was ruled out again (licence exposure, invariant 9, plus the most fragile capture path — their embed already refuses CME symbols, #328), so the chart keeps **our data** and adopts **their renderer**: lightweight-charts v5.0.9, Apache-2.0, vendored into `app/assets/` and served from this site — no CDN, no request to any third party, licence file shipped alongside and the library's attribution logo left visible. Layer 11b now stores the full session bar (open/high/low beside close and volume, all-or-nothing by the cleaner), so the panel draws real candles with a volume underlay, crosshair and zoom; closes-only histories draw as a line, sparse ones as dots, and the server decides which (`data-regime`) so no threshold lives in the page script. The inline SVG stays as the no-bundle fallback, and the bundle itself is in the promotion contract's `PUBLISHED_ASSETS` so a build that loses it cannot promote |
