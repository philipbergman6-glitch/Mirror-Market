...
**Ours, on top** — paper `#FAFAF7` (`--paper`) with a 3px `--green` left border. A mono 10px uppercase `--green` stamp rendered whole from `app/workstation_page.py` — `Our data · delayed daily closes · USD/MT · history since 7 Aug 2026 (9 sessions)` — over a small inline-SVG close-history line for that contract, drawn from `forward_curve` snapshots in the ledger drill-down's chart idiom (600×140 viewBox, observed-extreme axis labels, soy-green line and dots). The history is thin by construction (snapshots begin 2026-07-30) and the stamp says when it starts rather than letting a short axis imply a long record. The ledger's sparseness rule applies: **under 8 sessions dots only, never a joined line; under 2 no chart at all** — a stated reason in muted text instead (invariant 2). Points are keyed by session, so a weekend re-fetch of Friday's close is one point.
