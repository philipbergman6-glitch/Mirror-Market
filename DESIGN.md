...
**Ours, on top** — paper `#FAFAF7` (`--paper`) with a 3px `--green` left border. A mono 10px uppercase `--green` stamp rendered whole from `app/workstation_page.py` — `Our data · delayed daily closes · USD/MT · history since 7 Aug 2026 (9 sessions)` — over a small inline-SVG close-history line for that contract in the ledger drill-down's chart idiom (600×140 viewBox, observed-extreme axis labels, soy-green line and dots). The series is the contract's own Layer 11b daily history (months deep, [#332](https://github.com/philipbergman6-glitch/Mirror-Market/issues/332)), with the Layer 11 `forward_curve` snapshots filling any session the dedicated series lacks — same venue, same delayed-close vocabulary, so the merge is one kind of number. The stamp says when the record starts rather than letting a short axis imply a long record. The ledger's sparseness rule still applies whenever the history *is* thin (a fresh database, a just-listed month): **under 8 sessions dots only, never a joined line; under 2 no chart at all** — a stated reason in muted text instead (invariant 2). Points are keyed by session, so a weekend re-fetch of Friday's close is one point.
...
The chevron and row affordances are gated on a `.tv-ready` class the script adds after wiring — with script off the page loses the expander cleanly rather than offering a control that does nothing, the same gate `.drill-ready` applies on market pages. The SVG is drawn on first expand from JSON already in the page — nothing is requested from anywhere, so there is no failure state to render. A row whose venue has no checked symbol gets no expander at all (`app/tradingview.py`): a link to the wrong contract beside a right price is the same invariant-11 trade the wrong chart was.
