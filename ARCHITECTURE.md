...
- `river_levels` keys on `(gauge, Date)` and carries **two providers in one shape** (Layers 27/28 — NOAA NWPS in feet, Argentina INA in metres): the market stays a parameter via the `config.RIVER_GAUGES` registry's `provider` discriminator and each market descriptor's `river_gauges` list, while the two fetchers are graded as separate layers so an Argentine outage cannot take the Mississippi leg down. `stage` is never a price and never enters `to_usd_mt`; `unit` is stamped per row. Rendered as a compact line inside **block 06 (Weather)** — river water is tradeable weather, and a tenth block would be a permanent unfillable blank on six of eight markets.
...
#### Same-PK divergence quarantine (`pipeline/divergence.py`, T19 · F9 #67)

INSERT OR REPLACE is what makes the pipeline re-runnable and also what lets one corrupted fetch destroy good data: a bad close for a date already stored overwrites the good one under the same key, silently. `_save()` — the single choke point every table write passes through, which is why the guard hangs there and not off the thirty `save_*` bodies — screens each incoming row against **the value already stored under its own primary key**. Past `SAME_PK_DIVERGENCE_QUARANTINE_THRESHOLD` (20%, matching the trust ledger's `DAILY_MOVE_QUARANTINE_THRESHOLD`) the row is held back, appended to `quarantined_revisions` in the write's own transaction, and logged at ERROR. The stored value stands.

Four narrownesses are deliberate:

- **Same key, not same neighbourhood.** A 25% *session* is a fact about the market and must store — `clean._validate_price_data` keeps warning about it and nothing drops it. A 25% revision of a session already stored is a fact about the fetch. Only the second is caught here, and the two vocabularies stay separate on purpose.
- **One value column per table, whole row held.** The close is what every downstream analysis reads, so it is what is screened; the rest of the row travels with it, because writing a bar's high while holding its close back would leave a candle nobody could reconcile.
- **Registered tables only** (`divergence.GUARDED_TABLES`): `prices` and `currencies` from the ticket, plus the snapshot-only *price* tables `brazil_spot_prices`, `safex_prices` and `india_domestic_prices` — those upstreams serve only the current session, so the committed CSV is the only copy and an overwrite is unrecoverable. A weather series, a tonnage or a positioning report legitimately swings on revision; a guard that fired on them would train its readers to ignore it. The screen also declines, with a warning, when a caller keys a registered table differently from the registry — comparing on a key that is not the row's identity would screen unrelated observations.
- **Quarantine, not crash.** The good rows in the same frame still write; a corrupt upstream day does not take the run down. What a crash would protect, the stored value already survives.

Erring toward writing is the rule everywhere the comparison is not a measurement: a first observation, a stored `0`, a stored NULL and an incoming NULL all pass. A guard that swallowed legitimate rows would be a silent failure of its own kind.

`quarantined_revisions` keys on `(table_name, row_key, value_column, incoming_value)`, so replaying the same corrupted fetch rewrites one row while a *different* bad value is kept beside it, and carries the whole rejected row as JSON so the verdict can be argued with. It is deliberately **not** in `HISTORY_TABLES` and no surface renders it — a quarantined value is a number the store refused to believe, and `query.read_quarantined_revisions()` is an inspection seam for after an alert, not a display source.

**Known gap.** On the ephemeral CI database the self-healing tables (`prices`, `currencies`) start empty each morning, so nothing is stored to diverge from and the guard is inert there by construction; it protects the local database and, via the history CSVs, the three snapshot-only tables. A CI quarantine currently surfaces only as an ERROR line — it does not reach `pipeline_status.json` or the `ci-alert` issue, and the record itself dies with the runner. Plumbing a held-row count up to the run summary is the follow-up.

#### Git-based history persistence (`pipeline/history.py`)

CI runs on an ephemeral runner with an empty DB each day. Most layers self-heal by re-downloading full history, but snapshot-only sources don't: AgRural (1 row/day — the Brazil basis source), SAFEX, forward curve, CONAB survey revisions, inspections (>3 weeks) incl. port/destination breakdowns, Gulf bids, Argentina FOB (MAGyP serves history but re-fetch depth is unproven), CEPEA (>~10 sessions), WASDE (>12 months), India mandi (current-day snapshot), **export sales** (ESR is fetched for the current marketing year only, so the outgoing year vanishes at each MY rollover), **briefings** (generated from that run's DB; a past day's `text`/`snapshot_json` is reconstructible from no source at all), **SAGIS monthly supply & demand** (only the current season plus the two most recent finals are ever listed; older seasons exist only as per-month announcement files this layer does not fetch, so a season scrolling off the page is unrecoverable), and **SAGIS deliveries** (the DT export *does* serve history, but only a fixed 9-season window — nothing upstream promises it grows, so a season that rolls off is unrecoverable from an ephemeral CI DB). These tables round-trip through CSVs in `data/history/` (committed to git): `main.py` calls `import_history()` after `init_database()` (insert-if-absent on a NULL-safe primary-key match — DB rows win over CSVs; a blank cell reads back as `""` in a NOT NULL text column and as NULL in a nullable one, since CSV has one blank and SQLite has two) and `export_history()` after the layers (atomic per-table writes, PK-sorted for stable diffs). The deploy workflow commits `data/history/` back to `main` with `[skip ci]`. A failed import aborts the run so a bad seed can never be exported over good history. Cloud DB (Turso/Supabase) was explicitly rejected for this — do not reintroduce it as a CI requirement.
...
**The same-day second shot (#254).** Persistence only helps a row that was fetched at all. A snapshot-only upstream serves the current day and overwrites it tomorrow, so the daily build gets exactly one attempt and a single layer failure loses that observation permanently — the 2026-08-05 Gulf bid edition died on one parse error inside a run GitHub showed as green (a hard layer failure does not fail the run), and 3 of India mandi's first 4 days died on a throttled key. `retry-failed-pipeline.yml` runs `scripts/ci_retry_decision.py` on its own 20:40 UTC cron: it reads the `pipeline-status` artifact of the day's **last completed** deploy run and, if `hard_failures` is non-empty (or the summary is missing at all — the pipeline crashed), sends a `repository_dispatch: pipeline-retry` back to the deploy workflow for one full re-run. The pipeline is idempotent, so the retry's normal history-snapshot path stores whatever the upstream still serves.

Four properties are structural, not conventional. **One definition of failure**: the decision reads the same `hard_failures` key `ci_layer_alert.py` keys its outage issue on — there is no third notion of "failed". **No chains**: runs never trigger retries, the cron does, once per weekday — and the decision refuses when today already holds a `repository_dispatch` run, whatever its conclusion, the marker being the trigger type itself. **No load on green days**: a clean day's decision touches no upstream and exits in seconds, which is why this is a conditional trigger and not a second unconditional cron (that would double the data.gov.in sample key's load every day, including the days nothing was wrong). **Never past midnight**: past 23:20 UTC the decision refuses, because the upstreams are about to roll to tomorrow's edition and a late retry would recover nothing while consuming tomorrow's one retry. If the primary has not finished when the decision lands (both crons carry GitHub's +64–298 min drift), it defers rather than guessing — a day without a retry, not a wrong one.
...
- `app/block_builders.py` — the nine builders that fill that envelope, all **generic SQL over the registry descriptor**: `Source` names the table, date column, key column, value column and unit, so a tenth market is a `config.MARKETS` entry and no code. `Source.to_usd_mt` is the site's **only** conversion site — `native_exchange` (cents/bu, cents/lb, $/short ton), `usd_per_bushel` (AMS 3147 prints flat CIF bids in $/bu), `home_per_mt` (× the `<CCY>/USD` rate **of that row's own date**) or `usd_per_mt`. A `home_per_mt` leg with no FX rate renders its USD/MT as blank, never the local number relabelled. **The venue's own print is labelled with the venue's own unit, never the market's `home_currency`** — the two coincide only under `home_per_mt`. `Source.quote_unit` is the single label site for block 01 and the ledger alike, and `Source.has_native_quote` decides whether a second line exists at all: a `usd_per_mt` leg (Argentina's MAGyP FOB, the EC rapeseed workbook) is one observation in one unit, so it states its USD/MT figure once and carries **no** home-currency line — ARS/EUR is a number this stack never received. Block 01 previously labelled every leg `<home_currency> <n>/MT` unconditionally: Argentina printed `ARS 451.67/MT` against a dollar figure (out by the exchange rate, ~1,400×) and CBOT's three native units all printed as `USD …/MT` (out by 2.7×, and plausible) — #230. The registry is not the place to forbid the combination: `home_currency` describes the market and `usd_per_mt` describes the leg, and a peso-home market publishing an official dollar FOB is a true configuration, not a broken one. Crush and basis are struck on a session **all** legs printed — no cross-day arithmetic — and a home-currency crush margin is computed only where the legs share one per-MT currency. Several rows for one key on one date (AMS barge locations, MAGyP shipment windows) are averaged and the count is rendered.
...
- `.github/workflows/retry-failed-pipeline.yml` — the conditional same-day re-run for snapshot-only sources; decides only, never fetches.
...
**A row opens into its own history** (M11 #160 decided the shape, M21 #250 built it). Clicking a ledger row on a *market page* expands an inline panel under it — one at a time, never a modal, because the table's claim is comparative and a modal covers the rows being compared against. Four things it must keep straight:

- **The chart reads prints, not rows.** The series comes from `leg_prints`, the same read the row above it uses, so `trade_proof_column` applies at chart level too. A dot is a stronger claim than a table cell — it *looks* like evidence that something printed — so #157 costs more here than in the row.
- **Sparse history is drawn as dots, or not drawn.** By **observation count on the leg** (`LEDGER_LINE_MIN_OBS = 8`, `LEDGER_CHART_MIN_OBS = 3`): ≥8 is a line plus a dot per print, 3–7 is dots with **no connecting line**, under 3 is no chart at all — the level, the print date and where history starts, as text. A count rather than a span, because the snapshot legs are dense-but-short while a weekly leg is sparse-but-long. Every window states its own count, where it starts drawing, how far back the page reads, what the cap removed and what could not be converted — each a separate clause, because none of them stands in for another. The same floor binds the dashed overlay and the FX line: a series too sparse to draw is not drawn, and the pane says so.
- **The overlay is the page's own pinned leg**, dashed — CEPEA on Brazil, No.2 on Dalian, never CBOT everywhere. **India draws none**: the overlay is emitted only where the ledger names a leg from another market, which is a structural read of the registry rather than a per-market branch, and India's has none because no foreign bean is connected to a mandi by trade. A dashed series under a mandi one would render the `policy_blocked` +66% as a gap that closes. It is a *drawn* comparison and never an inferred one — no correlation or lead-lag figure (#145 stays out of scope).
- **Two panels, because one line cannot say which thing moved.** SAFEX moved +0.03% locally and +0.44% in USD on one session. The second panel is the currency's own series, for `home_per_mt` legs only; a `usd_per_mt` leg gets one sentence saying there is no second currency, per the dual-quote rule above. Every point converts at **its own date's** rate **or is withheld** — never at the oldest rate on file, which is what `SiteContext.fx_on` falls back to. Invariant 7 is "that row's own date's rate or blank", and on a 260-point chart the fallback draws a stretch of the currency's movement under the venue's name. The pane counts what it can draw, not what it fetched, so the caption and the picture cannot disagree.

The payload ships inline (static build, no server), clipped at build time to `LEDGER_DRILLDOWN_MAX_OBS = 260` per leg and encoded as **parallel arrays** off one origin date; the pinned leg and each currency series are emitted once per block rather than copied onto every row, and the venue's home-currency series is not shipped at all because neither pane draws it. A currency series gets its own cap (the widest window in days), since a daily rate prints ~261 weekdays a year and the leg cap would truncate every 1y FX pane for no saving worth having. Windows are **30d / 90d / 1y** — never "all", which beside a capped series would be a lie about the data. Market pages carry their own tighter `MARKET_PAGE_SIZE_BUDGET_BYTES = 150_000`, since the 1.5 MB site budget would never notice these pages tripling. The **range band is server-rendered** into the collapsed row (`352–378 since 29 Jul`, and `4 obs since 06 Aug — no range yet` under 8 prints); only the expansion is client-side, so with script off the ledger loses the drill-down and keeps every number it states. The **headline ledger is untouched** — gated on an explicit flag the block passes, not inferred from `has_spread`: its rows are markets, the market cell is already the affordance, and there is no pinned leg to overlay.
...
- **A named contract is not a continuous series.** Different types, neither accepted where the other is expected; `ContinuousSeries.is_hedgeable` is always `False`. Where the stored named-contract history is shorter than `MIN_SESSIONS`, a stitched series is **withheld** rather than padded with the provider's own front month — the silent substitution this phase exists to prevent. The provider series is still shown, labelled `provider_front_month` and carrying "the provider does not publish its roll dates". Since B4 (#310) the stitcher has a deep-history input: `build_from_bars` reads Layer 11b's `contract_bars` (years per contract, not one curve snapshot per day), strikes each roll step from the same session's two closes, and drops history older than a roll it cannot joinably adjust across — the reason stated on `adjustment_note`. The ratio-adjusted output is what `analysis.loaders.enrich_with_technicals` feeds the technical stack (A4 #301); levels a human reads stay raw named-contract prices.
...
## Trusted ledger — required FX (`trust/fx_ingestion.py`, #194)

The third pilot, and the one that decides what a physical price *is* in dollars: Layer 7's four conversion-critical pairs (`fx-brl-usd`, `fx-cny-usd`, `fx-inr-usd`, `fx-zar-usd`, from `REQUIRED_FX_PAIRS`). Every `home_per_mt` leg converts at these, so a wrong rate is a wrong landed cost on a whole origin. `trust/fx.py` reads FX revisions; this module is the writer.

- **The cutoff, not a settlement.** Spot FX has no settlement, so the FX contracts carry no `settlement.confirmed` rule — and that is exactly the trap. An unfinished FX bar would be *accepted*, land in the trusted frame, and diverge from a v1 path that dropped it, on **every** pre-cutoff run. So the adapter applies the same time-based cutoff v1 applies, importing `fetchers._settlement.FX_SESSION` rather than restating the 17:00 New York rollover. Two definitions of "finished" that can drift apart is the duplication this direction exists to remove. What it does not do is forget: the dropped session is a durable `fx.session-unfinished` Finding at WARNING severity and is named on the ingestion result — v1's equivalent is a log line that scrolls away. The two pilots therefore differ deliberately: benchmarks *ingest and reject* an unfinished bar, FX *never prices* one, and each is the honest treatment for its market.
- **An inverted quote quarantines.** `fx.orientation` (REJECT) pins the identity to the pair's own convention — the domain enforces it too, so a `usd-per-brl` unit cannot carry a BRL/USD pair — and `fx.plausible` (QUARANTINE) catches the case that parses perfectly: 5.43 is USD per BRL, positive, finite, and would multiply every Brazilian landed cost by thirty. `fx.daily-move` quarantines past `DAILY_MOVE_QUARANTINE_THRESHOLD` (10%, tighter than the benchmark 20% because these four currencies have no exchange limit to clear). A genuine devaluation past it is held, not dropped, and the reconciliation report names it.
- **No volume.** Yahoo reports zero volume on FX bars — an artefact of a market with no central tape, not an observation that nothing traded. Nothing is recorded, which is why `currencies` has no volume column either (invariant 2).

**Reconciliation.** `scripts/reconcile_fx.py` runs daily in `reconcile-trusted` beside the other two reports. It downloads each pair **once**, unguarded, through `fetchers.yfinance.download_bars` — the seam split out of `fetch_one` for exactly this: an adapter handed a frame v1's guard has already trimmed can never fire its own cutoff, and would pass while proving nothing. The v1 half then applies `drop_unsettled_session` with the run's own instant rather than calling `fetch_one`, because `fetch_one` reads the wall clock and a run straddling the rollover would otherwise judge the two paths at two different moments. Both are pinned to **one session** — the newest one v1 would publish. Exit 0 reconciled or no session, 1 diverged, 2 upstream unavailable. Quarantined revisions and dropped unfinished sessions are *reported, never graded*.

Tests: `tests/test_trust_fx_ingestion.py` (capture, cutoff, orientation, quarantine, reconciliation), `tests/test_reconcile_fx.py` (the runner, including a pre-rollover run reconciling by both paths dropping the same bar). All network-free.
